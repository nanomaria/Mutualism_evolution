...
choice = 1;
sM = @(M) sum((ones(N_AMF,N_AMF)-diag(ones(1,N_AMF)))*M);
sP = @(P) sum((ones(N_plant,N_plant)-diag(ones(1,N_plant)))*P);
m = @(alpha) abs(alpha);
if (choice == 1)
    %% Competition between AMF and  1 Plant
    Gamma_m = @(M) Aa*M./(sM(M)+(sM(M)<=0));
    fp = @(alpha,beta,P,M) P.*(q_hp*rp + q_hp*sum(m(alpha).*M)./(d+P) ...
        -q_cp*beta.*sum(M.*Gamma_m(M)./(Gamma_m(M) + sM(M) + ((Gamma_m(M) + sM(M))<=0) )) ...
        -mup*P) ;
    fm = @(alpha,beta,P,M) M.*( (q_cm*P.*Gamma_m(M)./(Gamma_m(M) + sM(M) + ((Gamma_m(M) + sM(M))<=0) )...
        - q_hm*m(alpha).*P./(d+P)) - mui.*M );
elseif (choice == 2)
    %% Competition between AMF and  1 Plant
...
while (t<Tf)&&(sum(abs(MM_d_new-MM_d_old))>1e-6)
    ...
    Pnew = (Pold + dt*fp(ALPHA',BETA',Pold,Mold));
    Mnew = (Mold + dt*fm(ALPHA',BETA',Pold,Mold));
